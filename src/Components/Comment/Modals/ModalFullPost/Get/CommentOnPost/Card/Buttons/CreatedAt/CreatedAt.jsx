...
const CreatedAt = ({ commentItem }) => {
  const [createdAt, setCreatedAt] = useState("");

  useEffect(() => {
    let dateNow = Date.now();
    let createdBefore = (dateNow - commentItem.createdAt) / 1000;
    if (createdBefore < 60) {
      if (createdBefore < 2) {
  ...
  return (
    <button className="modalFullPostCommentGetCreatedAt1">
      <p className="modalFullPostCommentGetCreatedAtText1">{createdAt}</p>
    </button>
  );
};
